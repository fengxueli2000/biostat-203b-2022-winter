...
`read.csv`read tabular data into R
...
icustays_tble %>% distinct(intime)
```

there are 53150 unique subject id and 76535 unique ICU time stays. Because the there are more time stays than subject id, so subject id can have multiple time stays.
...
```{r}
year_sum <- mutate(admissions_tble,
                     year =year(ymd_hms(admissions_tble$admittime))) %>%
  ggplot(aes(x = year))+
  geom_bar()
year_sum
```
...
```{r}
month_sum <- mutate(admissions_tble,
                     month =month(ymd_hms(admissions_tble$admittime))) %>%
  ggplot(aes(x = month))+
  geom_bar()
month_sum
```
...
```{r}
monthday_sum <- mutate(admissions_tble,
                     monthday =mday(ymd_hms(admissions_tble$admittime))) %>%
...
```{r}
weekday_sum <- mutate(admissions_tble,
                     monthday =wday(ymd_hms(admissions_tble$admittime))) %>%
  ggplot(aes(x = monthday))+
  geom_bar()
weekday_sum
```
...
```{r}
hour_sum <- mutate(admissions_tble,
                     hour =hour(ymd_hms(admissions_tble$admittime))) %>%
  ggplot(aes(x = hour))+
  geom_bar()
hour_sum
```
...
gender_sum <- ggplot(patients_tble, aes(x = gender)) +
        geom_bar()
gender_sum
```

we can see from the figure and table that men is a little more than women

```{r}
age_sum <- ggplot(patients_tble, aes(x = anchor_age)) +
        geom_bar()
age_sum
```
...
```{r}
labevents_tble <- left_join(labevents_tble,dlabitems_tble, by = c("itemid"))  
labevents_tble <- left_join(labevents_tble, icustays_tble, 
                            by = c("subject_id")) %>% 
...
labevents_tble %>% 
  ...
  summarise(mean_valuenum = mean(valuenum, na.rm = TRUE),
            number_valuenum = n(),
            median_valuenum = median(valuenum, na.rm = TRUE),
            sd_valuenum = sd(valuenum, na.rm = TRUE))
category_barchart <- ggplot(labevents_tble, aes(x = category)) +
        geom_bar()
category_barchart

label_barchart <- ggplot(labevents_tble, aes(x = label)) +
        geom_bar()
label_barchart
...
mimic_icu_cohort <- 
  ...
  mutate(thirty_day_mort = (deathtime - admittime) < 30)  
```
