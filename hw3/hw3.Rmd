...
```{r}
# full admission_table
admissions_tble <- tbl(con, "admissions") %>%
  show_query() %>%
  print(width = Inf)
# select patients have match
...
```{r}
# # TODO
chartevents_tble <- left_join(chartevents_tble, icustays_tble, 
                            by = c("subject_id")) %>% 
  filter(charttime >= intime) %>% 
  group_by(subject_id, itemid) %>% 
  slice_min(charttime) %>% 
...
icu_cohort$thirty_day_mort <- 
  ifelse(is.na(icu_cohort$dod), "No",
  ifelse(as.POSIXlt(icu_cohort$dod) - icu_cohort$admittime <= 2592000,
                                            "Yes", "No"))
icu_cohort %>%  write_rds("icu_cohort.rds")
```
